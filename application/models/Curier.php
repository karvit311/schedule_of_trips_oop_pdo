...
class Curier 
{
    ...
    {  
        $conn =App::$app->get_db(); 
        return $conn->query("SELECT * FROM curier")->fetchAll(); 
    }      
    public function get_prepare($new_curier)
    {
        $conn =App::$app->get_db(); 
        $stmt =$conn->prepare("SELECT id, name FROM curier WHERE name = ?");
    ...
    {
        $conn =App::$app->get_db(); 
        $stmt =$conn->prepare("SELECT id, name FROM curier WHERE id = ?");
    ...
    {
        $conn =App::$app->get_db();
        $stmt = $conn->prepare( "INSERT INTO curier (name)  VALUES(:name)");
